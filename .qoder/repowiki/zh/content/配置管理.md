...
- 引导与注入：Bootstrap 在启动时将配置作为服务注入缓存、文件系统等模块。
...
- 数据库配置（Database）
  - 默认连接名、连接集合（驱动、主机、端口、用户、密码、库名、SSL 模式、表前缀）。
- 缓存配置（Cache）
...
### 配置在各模块中的使用
- Bootstrap 注入：启动时将全局配置注册为服务，供缓存、文件系统等模块使用。
...
## 结论
CMF 的配置管理以 Viper 为核心，结合默认值、配置文件与环境变量，形成清晰的加载与优先级机制。通过 Bootstrap 将配置注入到缓存、文件系统、JWT 等模块，实现了模块间的解耦与统一管理。建议在实际使用中遵循环境隔离、最小暴露与变更审批的最佳实践，确保配置的安全与稳定。
