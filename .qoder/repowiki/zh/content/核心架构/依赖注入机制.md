...
- [provider.go](file://filesystem/provider.go)
...
## 更新摘要
**变更内容**
- 新增文件系统模块的Google Wire依赖注入支持，filesystem/provider.go引入了ProviderSet
- 更新Google Wire集成章节，增加文件系统模块的ProviderSet定义
- 更新核心组件部分，反映文件系统模块的Wire集成增强
- 新增文件系统模块在Wire依赖解析中的作用说明
...
## 简介
本文件围绕 CMF 框架的依赖注入机制展开，重点解释 Bootstrap 如何实现 IoC 容器功能，详细描述 RegisterService 方法的单例模式与 sync.Map 的并发安全设计；阐述服务注册、查找、类型转换的完整流程，并对比泛型服务获取（GetServiceTyped、MustGetServiceTyped）在安全性与便利性方面的差异；分析服务依赖关系的管理方式及如何通过依赖注入实现模块间的解耦；**新增文件系统模块的Google Wire依赖注入支持，通过ProviderSet增强模块间的依赖管理能力和测试友好性**；最后给出服务注册的最佳实践与常见使用场景，包括核心服务（config、cache、filesystem）的自动注册机制。

## 项目结构
CMF 采用模块化设计，核心依赖注入位于 bootstrap 包，配合 config、cache、filesystem 等模块完成服务的自动注册与运行时获取。**新增文件系统模块的Google Wire集成支持，filesystem/provider.go通过ProviderSet提供自动依赖解析能力**。README 明确指出"依赖注入：使用 Bootstrap 结构体和服务注册模式（RegisterService、GetService、MustGetServiceTyped）管理依赖"。
...
FS["Filesystem ProviderSet<br/>NewFilesystemFromConfig"]
end
subgraph "第三方集成"
V["Viper<br/>配置解析"]
...
- [provider.go:5-6](file://filesystem/provider.go#L5-L6)
...
- **ProviderSet：Google Wire 集合，提供自动依赖解析和模块化服务注册能力**，**新增文件系统模块的ProviderSet支持**。
...
## 架构总览
Bootstrap 通过 RegisterService 将核心服务注册为单例，随后在 Run、init、loadMiddlewares、setupRoutes 等阶段通过 MustGetServiceTyped 获取强类型服务实例，实现模块间解耦与集中管理。**新增文件系统模块的Google Wire集成后，模块可通过ProviderSet实现自动依赖解析，简化复杂依赖的管理，增强测试友好性**。
...
- Filesystem：
  ...
  - **新增ProviderSet支持，通过NewFilesystemFromConfig提供自动依赖解析**。
...
### ProviderSet 概述
CMF 框架集成了 Google Wire 依赖注入框架，为各模块提供自动依赖解析能力。每个模块通过 ProviderSet 暴露其服务提供者，Wire 可以自动解析和注入依赖关系。**新增文件系统模块的ProviderSet支持，通过NewFilesystemFromConfig函数提供自动依赖解析能力**。
...
- **Filesystem 模块**：**新增**提供 NewFilesystemFromConfig 函数的 ProviderSet

### 文件系统模块的Wire集成
文件系统模块通过以下方式实现Wire集成：
...
**功能特性**：
- **自动依赖解析**：Wire自动解析NewFilesystemFromConfig函数的依赖关系
- **配置驱动**：基于config.Config自动创建合适的文件系统实例
- **双写支持**：自动处理IsAndLocal配置下的双存储适配器创建
- **测试友好**：支持在测试环境中轻松替换依赖
...
wire.Build(
    ...
    filesystem.ProviderSet, // 新增文件系统模块
)
```
...
- **Wire 集成后，模块间依赖关系通过 ProviderSet 自动解析，支持更复杂的依赖图管理，新增文件系统模块的ProviderSet增强了整体依赖管理能力**。
...
Wire --> ORM["ORM"]
Wire --> VAL["Validate"]
Wire --> FSP["Filesystem ProviderSet"]
Cch --> Cfg
FS --> Cfg
Cch --> BC["BigCache Driver"]
Cch --> RD["Redis Driver"]
FS --> DS["DualStorage Adapter"]
```
...
- **Wire 集成性能**：ProviderSet 在编译时解析，运行时开销极小；复杂依赖图的构建在编译时完成。**文件系统模块的ProviderSet通过NewFilesystemFromConfig提供高效的依赖解析**。
...
- **文件系统模块Wire集成问题**：
  - 症状：NewFilesystemFromConfig无法被Wire解析
  - 排查：确认config.Config依赖正确；检查filesystem配置项；验证ProviderSet定义。
...
## 结论
Bootstrap 通过 sync.Map 实现线程安全的单例容器，结合 RegisterService、GetService、MustGetServiceTyped 等方法，提供类型安全与便利性的双重保障。核心服务（config、cache、filesystem）在启动阶段自动注册，模块通过服务名与类型解耦，实现清晰的依赖注入与模块化架构。**新增的Google Wire集成和文件系统模块的ProviderSet支持进一步增强了依赖注入能力，通过ProviderSet提供自动依赖解析和模块化服务注册，简化了复杂依赖的管理，提升了开发效率和代码质量，特别是文件系统模块的自动依赖解析能力为测试友好性和模块解耦提供了更好的支持**。配合配置驱动的缓存与文件系统切换，CMF 能够在不同部署环境下灵活适配。
...
- **文件系统模块**：在需要自动依赖解析的场景使用ProviderSet，通过NewFilesystemFromConfig简化配置驱动的文件系统创建。
...
- **文件系统模块场景**：通过filesystem.ProviderSet自动解析文件系统依赖，简化测试环境配置；利用NewFilesystemFromConfig的配置驱动特性。
